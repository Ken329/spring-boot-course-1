...
import dev.kenl.runnerz.Application;
...
public class RunRepository {
    ...
    public void create(Run run) {
        var created = jdbcClient.sql("INSERT INTO Run(id, title, started_on, completed_on, miles, location) VALUES (?,?,?,?,?,?)").params(List.of(run.id(), run.title(), run.startedOn(), run.completedOn(), run.miles(), run.location().toString())).update();
        Assert.state(created == 1, "Failed to create run" + run.title());
    }

    public void bulkCreate(List<Run> runs) {
    ...
    public void delete(Integer id) {
        var deleted = jdbcClient.sql("DELETE FROM RUN WHERE id = :id").param("id", id).update();
